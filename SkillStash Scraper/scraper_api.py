...
# This instructs GPT to strictly use your provided categories
SYSTEM_PROMPT = """
...
def ask_openai_for_metadata(title, instructor):
    """Sends title to GPT to get Description, Tech, Category, and Subcategory."""
    try:
...
def scrape_discudemy_course(url):
    ...
    if udemy_link_element and 'href' in udemy_link_element.attrs:
        ...
        try:
            go_response = requests.get(intermediate_url, headers=headers)
    ...
    # Instead of scraping empty fields, we ask GPT to fill them based on the Title
    ...
    return {
        'title': title,
        'description': ai_data.get('description'), # AI Generated
        'instructor': instructor,
        'technologies': ai_data.get('technologies'), # AI Generated
        'originalPrice': original_price,
        'discountedPrice': discounted_price,
        'imageUrl': image_url,
        'udemyUrl': udemy_url,
        'category': ai_data.get('category'), # AI Generated
        'subcategory': ai_data.get('subcategory') # AI Generated
    }
...
if __name__ == '__main__':
    print("AI-Enhanced Scraper API running on http://127.0.0.1:5000")
